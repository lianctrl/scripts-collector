...
if [ -e "$date""_1M_elf_""$Electric_Field" ]; then
...
mkdir "$date""_1M_elf_""$Electric_Field" 


sed -e "s/XXXX/$Electric_Field/g" $folder"electric_field_try-XXXX.mdp" > "$date""_1M_elf_""$Electric_Field""/"$OUTPUTMDP
...
cp $folder$topology_file "$date""_1M_elf_""$Electric_Field""/tmp_file_0.itp"
cp $folder* "$date""_1M_elf_""$Electric_Field""/"
...
#	echo "$B"
	...
	cd ../"$date""_1M_elf_""$Electric_Field""/"
...
#echo "You have successfully created the new modified topology named topol_ch_7_7_60_MODIFIED.itp !! GOOD LUCK!"
